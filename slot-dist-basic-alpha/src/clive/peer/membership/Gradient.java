...
public final class Gradient extends ComponentDefinition {
	...
	private HashMap<Integer, Double[]> history = new HashMap<Integer, Double[]>();
	private int index = 0;
...
//-------------------------------------------------------------------	
	public Gradient() {
		...
		subscribe(handleHitsTimeout, timerPort);
...
//			SchedulePeriodicTimeout ht = new SchedulePeriodicTimeout(50000, 50000);
//			ht.setTimeoutEvent(new HitsTimeout(ht));
//			trigger(ht, timerPort);
...
//-------------------------------------------------------------------	
	private void initiateShuffle(int shuffleSize, MSPeerAddress randomPeer) {
		...
		ShuffleRequest rRequest = new ShuffleRequest(self, randomPeer, rTimeoutId, randomBuffer, slots);
...
//-------------------------------------------------------------------	
	Handler<ShuffleRequest> handleRandomShuffleRequest = new Handler<ShuffleRequest>() {
		public void handle(ShuffleRequest event) {
			...
			ShuffleResponse response = new ShuffleResponse(self, peer, event.getRequestId(), toSendRandomBuffer, slots);
...
//-------------------------------------------------------------------	
	Handler<ShuffleResponse> handleRandomShuffleResponse = new Handler<ShuffleResponse>() {
		public void handle(ShuffleResponse event) {
			...
			//System.out.println(self + ": timestamp: " + timestamp + ", " + event.getMSPeerSource() + ", peer timestamp: " + event.getTimestamp());
...
//-------------------------------------------------------------------	
	public void reset() {
		Double[] iterHits = new Double[Configuration.SLOT_RANGE];
		
		for (int i = 0; i < Configuration.SLOT_RANGE; i++)
			iterHits[i] = hits[i];
				
		history.put(index, iterHits);
		index++;
		if (index == Configuration.ALPHA)
			index = 0;			
...
//-------------------------------------------------------------------	
    private void findDistribution() {
    	double count = 0;
    	double[] sumHistory = new double[Configuration.SLOT_RANGE];
    	double[] historyHists = new double[Configuration.SLOT_RANGE];

		for (int i = 0; i < Configuration.SLOT_RANGE; i++)
			sumHistory[i] = 0;
    	
    	for (Double[] h : history.values()) {
    		for (int i = 0; i < Configuration.SLOT_RANGE; i++)
    			sumHistory[i] += h[i];
    	}
		...
		for (int i = 0; i < Configuration.SLOT_RANGE; i++)
			historyHists[i] = sumHistory[i] / count;

		Snapshot.updateNum(self, historyHists);		
    }
	
//-------------------------------------------------------------------	
//	public void calcHits() {
//		Double[] iterHitRatio = new Double[Configuration.SLOT_RANGE];
//		findLocalDistribution();
//		
//		for (int i = 0; i < Configuration.SLOT_RANGE; i++)
//			iterHitRatio[i] = hitRatio[i];
//				
//		history.put(index, iterHitRatio);
//		index++;
//		if (index == Configuration.ALPHA)
//			index = 0;			
//		
...
//
////-------------------------------------------------------------------	
//    private double[] findHistoryHits() {
//    	double count = 0;
//    	double[] sumHistory = new double[Configuration.SLOT_RANGE];
//    	double[] historyHists = new double[Configuration.SLOT_RANGE];
//
