...
//-------------------------------------------------------------------	
	private void initiateShuffle(int shuffleSize, MSPeerAddress randomPeer) {
		...
		double[] nodeHits = new double[Configuration.SLOT_RANGE];
		for (int i = 0; i < Configuration.SLOT_RANGE; i++)
			nodeHits[i] = hits[i];
				
		ShuffleRequest rRequest = new ShuffleRequest(self, randomPeer, rTimeoutId, randomBuffer, nodeHits);
...
//-------------------------------------------------------------------	
	Handler<ShuffleRequest> handleRandomShuffleRequest = new Handler<ShuffleRequest>() {
		public void handle(ShuffleRequest event) {
			...
			ShuffleResponse response = new ShuffleResponse(self, peer, event.getRequestId(), toSendRandomBuffer, nodeHits);
			trigger(response, messagePort);
			
			double[] peerHits = event.getHits();
			findHitsDistribution(peerHits);
			findLocalDistribution();
		}
	};
...
//-------------------------------------------------------------------	
    private void findHitsDistribution(double[] peerHits) {
		for (int i = 0; i < Configuration.SLOT_RANGE; i++)
			hits[i] += peerHits[i];
    }
}
